...
export const BlogModal: React.FC<BlogModalProps> = ({
  ...
  const [category, setCategory] = useState('Tips & Info');
  const [author, setAuthor] = useState('Admin UMKM');
  ...
  useEffect(() => {
    if (blog) {
      setTitle(blog.title || '');
      setCategory(blog.category || 'Tips & Info');
      setAuthor(blog.author || 'Admin UMKM');
      ...
      setTitle('');
      setCategory('Tips & Info');
      setAuthor('Admin UMKM');
      setExcerpt('');
      setContent('');
      setImageUrl('');
      setImageFile(null);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl max-w-2xl w-full p-6 space-y-4 shadow-2xl my-8">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 className="text-base font-bold text-slate-800">
            {blog ? 'Edit Artikel Blog' : 'Tulis Artikel Baru'}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          <div>
            ...
            <input
              type="text"
              required
              placeholder="misal: 5 Cara Memilih Batik Tulis Solo Asli"
              value={title}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-slate-700 mb-1">Kategori Blog</label>
              <input
                type="text"
                value={category}
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Ringkasan (Excerpt)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Isi Artikel Lengkap *</label>
            <textarea
              rows={6}
              required
          ...
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Gambar Utama Artikel</label>
            <input
              type="file"
              accept="image/*"
